...
class AgendamentoController(private val context: Context) {
    ...
    suspend fun listar(pagina: Int, itens: Int): List<AgendamentoResponse> {
        ...
        try {
            ...
            val response = agendamentoService.listar(
                pagina = pagina,
    ...
    suspend fun listarAgendamentosByVeterinario(idVeterinario: Long): List<AgendamentoResponse> {

        val endpoint = "listarAgendamentosByVeterinario";

        Log.i(TAG, "[$endpoint] - Inicio (VetID: $idVeterinario)");

        try {

            val response = agendamentoService.listarPorVeterinario(idVeterinario);

            if (response.isSuccessful) {

                val agendamentos = response.body() ?: emptyList();

                Log.i(TAG, "[$endpoint] - Sucesso. ${agendamentos.size} agendamentos encontrados do veterinário com id $idVeterinario.");
                Log.i(TAG, "[$endpoint] - Fim");
